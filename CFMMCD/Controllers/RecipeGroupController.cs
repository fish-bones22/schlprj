...
{
    public class RecipeGroupController : Controller
    {
        // GET: RecipeGroup
        public ActionResult Index()
        {
            ...
            Session["CurrentPage"] = new CurrentPageSession("GRP_RIR", "HOME", "LOG");


            ItemGroupViewModel IGViewModel = new ItemGroupViewModel();
            IGViewModel.GroupType = 4;
            IGViewModel.RecipeList = ItemGroupManager.GetFilteredRecipe();
        ...
        public ActionResult AddNewGroup(ItemGroupViewModel IGViewModel)
        {
            ...
            if (result)
            {
                ...
                new AuditLogManager().Audit(user.Username, DateTime.Now, "Recipe Group", PageAction, IGViewModel.GroupId.ToString(), IGViewModel.GroupName);
            }
            else
            {
        ...
        public ActionResult DeleteGroup(string value)
        {
            ...
            if (result)
            {
                ...
                new AuditLogManager().Audit(user.Username, DateTime.Now, "Recipe Group", PageAction, value, Name);
            }
            else
            {
                TempData["ErrorMessage"] = PageAction + " failed";
