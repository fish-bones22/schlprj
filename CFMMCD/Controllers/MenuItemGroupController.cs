...
namespace CFMMCD.Controllers
{
    public class MenuItemGroupController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            Session["CurrentPage"] = new CurrentPageSession("GRP_MIM", "HOME", "LOG");


            ItemGroupViewModel IGViewModel = new ItemGroupViewModel();
            IGViewModel.GroupType = 1;
            IGViewModel.MenuItemList = ItemGroupManager.GetFilteredMenuItem();
        ...
        public ActionResult AddNewGroup(ItemGroupViewModel IGViewModel)
        {
            ...
            if (result)
            {
                ...
                new AuditLogManager().Audit(user.Username, DateTime.Now, "Menu Item Group", PageAction, IGViewModel.GroupId.ToString(), IGViewModel.GroupName);
            }
            else
            {
        ...
        public ActionResult DeleteGroup (string value)
        {
            ...
            if (result)
            {
                ...
                new AuditLogManager().Audit(user.Username, DateTime.Now, "Menu Item Group", PageAction, value, Name);
            }
            else
            {
                TempData["ErrorMessage"] = PageAction + " failed";
