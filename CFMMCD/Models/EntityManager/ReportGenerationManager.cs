...
namespace CFMMCD.Models.EntityManager
{
    public class ReportGenerationManager
    {
        ...
        public bool ExportToPDF(MenuItemMasterViewModel MIMVM, MenuRecipeViewModel MRVM, RawItemMasterViewModel RIMVM, StoreProfileViewModel SPVM)
        {
            using (CFMMCDEntities db = new CFMMCDEntities())
            {
                using (DataSet ds = new DataSet())
                {
                    ...
                    gv3.AllowPaging = false;
                    gv3.DataSource = dt3;
                    gv3.DataBind();
                    GridView gv4 = new GridView();
                    gv4.AllowPaging = false;
                    gv4.DataSource = dt4;
                    gv4.DataBind();


                    HttpContext.Current.Response.ContentType = "application/pdf";
                    HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=ReportGeneration.pdf");
                    HttpContext.Current.Response.Cache.SetCacheability(HttpCacheability.NoCache);
                    StringWriter sw = new StringWriter();
                    HtmlTextWriter hw = new HtmlTextWriter(sw);
                    gv.RenderControl(hw);
                    gv2.RenderControl(hw);
                    gv3.RenderControl(hw);
                    gv4.RenderControl(hw);
